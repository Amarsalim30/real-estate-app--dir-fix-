...
const unitTypes = useMemo(() => {
  ...
  return [...new Set(units.map(unit => unit.type))];
}, [units]);
...
const filteredAndSortedUnits = useMemo(() => {
  ...
  return filtered;
}, [units, statusFilter, typeFilter, projectFilter, sortBy, sortOrder]);

  // Add this after all your hooks but before the return
if (isLoading) {
  return (
...
const getProjectName = (projectId) => {
  if (!projects || !Array.isArray(projects)) return 'Unknown Project';
  ...
  Showing {filteredAndSortedUnits?.length || 0} of {units?.length || 0} units
</p>
